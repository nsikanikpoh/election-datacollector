class PatriotsController < ApplicationController
...
if referrer_code.present?
  ...
  def update
    respond_to do |format|
      if @user.update(user_params)
        format.html { redirect_to @user, notice: 'Patriot was successfully updated.' }
  ...
  def destroy
    @user.destroy
    respond_to do |format|
      format.html { redirect_to users_url, notice: 'Patriot was successfully destroyed.' }
  ...
  private
    ...
    def user_params
      params.require(:patriot).permit(:referrer_code, :affiliate_code, :name, :birthday, :image, :remove_image, :image_cache, :title, :gender, :phone, :type, :address, :location, :state, :email, :password, :remember_me)
    end
end
