...
if referrer_code.present?
  if User.find_by_affiliate_code(referrer_code)
    @affiliate = User.find_by_affiliate_code(referrer_code)
     @user = Chamption.new(user_params)
    ...
    @user.save
     respond_to do |format|
      if @user.save
        ...
        ChamupJob.set(wait: 20.seconds).perform_later(@user)
        sign_in(@user)
        format.html { redirect_to root_path, notice: 'Champion Account was successfully created. Thank you!' }
  ...
  def update
    respond_to do |format|
      if @user.update(user_params)
        format.html { redirect_to @user, notice: 'Champion was successfully updated.' }
  ...
  def destroy
    @user.destroy
    respond_to do |format|
      format.html { redirect_to users_url, notice: 'Champion was successfully destroyed.' }
